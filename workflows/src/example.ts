...
export const workflow = ({ model, positive, negative, fast, batch }: {
    model: string,
    positive: string,
    negative?: string,
    fast?: boolean,
    ...
    const _iter = new comfy.KSampler({ model: _model.MODEL, positive: _clip_positive.CONDITIONING, negative: _clip_negative.CONDITIONING, scheduler: 'karras', sampler_name: 'euler', latent_image: _latent.LATENT, steps: fast ? 20 : 30 })
